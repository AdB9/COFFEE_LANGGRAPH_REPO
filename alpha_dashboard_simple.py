...
def main():
    ...
    with col1:
        ...
        try:
            st.image("data/evaluation_plots.png", caption="Model Performance Analysis", use_column_width=True)
        except:
    ...
    with col2:
        ...
        try:
            st.image("data/model_evaluation.png", caption="Error Distribution & Metrics", use_column_width=True)
        except:
    ...
    with col1:
        error_threshold = st.slider(
            ...
            max_value=30.0, 
            value=15.0,
            help="Minimum prediction error to analyze"
        )
    ...
    with col2:
        analysis_days = st.slider(
            ...
            max_value=30,
            value=10,
            help="Days around error events to search"
        )
    
    with col3:
        st.write("**Data Sources:**")
        enable_geospatial = st.checkbox("🛰️ Geospatial", value=True)
        enable_web_news = st.checkbox("📰 News", value=True)
        enable_logistics = st.checkbox("🚛 Logistics", value=False)
    ...
    if 'alpha_results' in st.session_state and st.session_state.alpha_results:
        ...
        for i, indicator in enumerate(alpha_indicators, 1):
            with st.expander(f"Alpha Indicator #{i}: {indicator['name']} (Confidence: {indicator['confidence_score']:.2f})"):
                ...
                with col1:
                    ...
                    if 'detailed_reasoning' in indicator:
                        st.markdown("**Detailed Analysis & Reasoning:**")
                        st.markdown(indicator['detailed_reasoning'])
                    
                    st.markdown(f"**Data Sources:** {indicator['data_source']}")
                    
                    # Show additional analysis details if available
                    if 'failure_correlation' in indicator:
                        st.markdown(f"**Failure Correlation:** {indicator['failure_correlation']}")
                    
                    if 'statistical_significance' in indicator:
                        st.markdown(f"**Statistical Significance:** {indicator['statistical_significance']}")
                
                with col2:
                    # Confidence gauge
                    fig = go.Figure(go.Indicator(
                        mode = "gauge+number",
                        value = indicator['confidence_score'],
        ...
        # Next Steps Section
        st.header("🚀 Next Steps: Using Alpha Features")
        
        st.markdown("""
        <div class="insight-box">
            <h4>🎯 Feature Integration Strategy</h4>
            <p>Your discovered alpha indicators can be implemented as additional features in your 
            commodity trading models. Each indicator provides specific data sources and signal 
            generation logic that can enhance prediction accuracy.</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            - [ ] Create alert systems for signal degradation
            - [ ] Document feature maintenance procedures
            
            ### 📈 Continuous Improvement
            - [ ] Monitor feature performance over time
            - [ ] Retrain models with new data
            - [ ] Discover additional alpha indicators
            - [ ] Expand to other commodity markets
            """)
        
        # Sample code integration
        with st.expander("💻 Sample Code Integration"):
            st.code(f"""
# Alpha Feature Integration Pipeline
import pandas as pd
import numpy as np
...
class AlphaFeatureEngineer:
    def __init__(self):
        self.alpha_indicators = {len(alpha_indicators)}  # Discovered indicators
        
    def create_alpha_features(self, base_data):
        \"\"\"Add alpha indicators as model features\"\"\"
        features = base_data.copy()
        
        # Example: Implement top indicator
        # {alpha_indicators[0]['name'] if alpha_indicators else 'Sample Indicator'}
        features['drought_stress_index'] = self.calculate_soil_moisture_deficit(
            base_data['date'], region='sao_paulo'
        )
        
        # Add lagged features for time series patterns
        for lag in [1, 3, 7]:
            features[f'drought_stress_lag_{{lag}}'] = features['drought_stress_index'].shift(lag)
        
        return features

# Usage in your model pipeline
feature_engineer = AlphaFeatureEngineer()
enhanced_features = feature_engineer.create_alpha_features(historical_data)

# Train enhanced model
model = RandomForestRegressor(n_estimators=100)
model.fit(enhanced_features, target_prices)
            """, language="python")
