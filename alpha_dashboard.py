...
def main():
    ...
    if 'alpha_results' in st.session_state and st.session_state.alpha_results:
        ...
        with st.expander("💻 Sample Code Integration"):
            # Get the first indicator name safely
            if alpha_indicators:
                if hasattr(alpha_indicators[0], 'name'):
                    first_indicator_name = alpha_indicators[0].name
                else:
                    first_indicator_name = alpha_indicators[0]['name']
            else:
                first_indicator_name = 'Sample Indicator'
